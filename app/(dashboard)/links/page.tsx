  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      <div className="space-y-3">
        ...
        {links?.map((link) => (
          <div key={link.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 flex items-center justify-between">
            <div>
              <div className="text-white text-sm font-medium">{link.label || link.context}</div>
              <div className="text-zinc-500 text-xs mt-0.5">persona.page/{link.slug}</div>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs text-zinc-600 capitalize">{link.context.replace('_', ' ')}</span>
